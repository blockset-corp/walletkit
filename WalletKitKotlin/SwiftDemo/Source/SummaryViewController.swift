...
class SummaryViewController: UITableViewController, WalletListener {
    ...
    func updateTitle () {
        self.navigationItem.title = "Testnet Wallets (\(UIApplication.paperKey.components(separatedBy: " ").first!))"
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        case "showManagers":
            ...
            controller.managers = UIApplication.sharedSystem.walletManagers
    ...
    @IBAction func doAddWallet(_ sender: Any) {
        ...
        let currencies = system.walletManagers
        ...
        currencies.forEach { (c) in
            // The manager for (c)`
            if let manager = system.walletManagers.first (where: { $0.network.hasCurrency(currency: c) }) {
    ...
    func handleWalletEvent (system: System,
        ...
        DispatchQueue.main.async {
            ...
            case is WalletEvent.BalanceUpdated:
