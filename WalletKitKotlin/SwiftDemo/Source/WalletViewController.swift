...
//            break
    ...
    func addAlertAction (alert: UIAlertController, _ canDisable: Bool, _ action: UIAlertAction) {
        if canDisable {
            action.isEnabled = (wallet.manager.network.type is NetworkType.BTC)
        }
    ...
    func handleTransferEvent(system: System, manager: WalletManager, wallet: Wallet, transfer: Transfer, event: TransferEvent) {
        DispatchQueue.main.async {
            ...
            case is TransferEvent.Changed: //  (let old, let new)
                if let index = self.transfers.firstIndex (of: transfer) {
                    let path = IndexPath (row: index, section: 0)
                    self.tableView.reloadRows(at: [path], with: .automatic)
                }

            case is TransferEvent.Deleted:
                if let index = self.transfers.firstIndex (of: transfer) {
