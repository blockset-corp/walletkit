...
public class CurrencyAIT {
    ...
    public void testCurrencyPair() {
        ...
        // BTC -> USD
        Optional<com.blockset.walletkit.Amount> oneBTCinUSD = pair.exchangeAsBase(Amount.create(1.0, btc_btc));
        assertTrue(oneBTCinUSD.isPresent());
        assertEquals (10000.0, oneBTCinUSD.get().doubleAmount(usd_dollar).get(), 1e-6);

        // USD -> BTC
        Optional<com.blockset.walletkit.Amount> oneUSDinBTC = pair.exchangeAsQuote(Amount.create(1.0, usd_dollar));
