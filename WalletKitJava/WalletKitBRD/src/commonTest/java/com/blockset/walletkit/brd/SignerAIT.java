...
import com.blockset.walletkit.brd.Key;
import com.blockset.walletkit.brd.Signer;
...
public class SignerAIT {
    ...
    public void testSigner() {
        ...
        digest = Hasher.createForAlgorithm(com.blockset.walletkit.Hasher.Algorithm.SHA256).hash(msg.getBytes(StandardCharsets.UTF_8)).get();
        signer = Signer.createForAlgorithm(com.blockset.walletkit.Signer.Algorithm.BASIC_DER);
        ...
        msg = "How wonderful that we have met with a paradox. Now we have some hope of making progress.";
        digest = Hasher.createForAlgorithm(com.blockset.walletkit.Hasher.Algorithm.SHA256).hash(msg.getBytes(StandardCharsets.UTF_8)).get();
        signer = Signer.createForAlgorithm(com.blockset.walletkit.Signer.Algorithm.BASIC_JOSE);
        signature = signer.sign(digest, key).get();
        ...
        msg = "foo";
        digest = Hasher.createForAlgorithm(com.blockset.walletkit.Hasher.Algorithm.SHA256).hash(msg.getBytes(StandardCharsets.UTF_8)).get();
        signer = Signer.createForAlgorithm(com.blockset.walletkit.Signer.Algorithm.COMPACT);
        signature = signer.sign(digest, key).get();
    ...
    public void testCompactSigner() {
        ...
        byte[] message = Hasher.createForAlgorithm(com.blockset.walletkit.Hasher.Algorithm.SHA256_2)
        ...
        for (int i = 0; i < secrets.length; i++) {
            ...
            byte[] outputSig = Signer.createForAlgorithm(com.blockset.walletkit.Signer.Algorithm.COMPACT).sign(message, maybeKey.get()).get();
            String outputSigHex = Coder.createForAlgorithm(com.blockset.walletkit.Coder.Algorithm.HEX).encode(outputSig).get();
