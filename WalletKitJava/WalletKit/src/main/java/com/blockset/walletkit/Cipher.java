...
public interface Cipher {
    ...
    static Cipher createForAesEcb(byte[] key) {
        return Api.getProvider().cipherProvider().createCipherForAesEcb(key);
    }

    static Cipher createForChaCha20Poly1305(Key key, byte[] nonce12, byte[] ad) {
        return Api.getProvider().cipherProvider().createCipherForChaCha20Poly1305(key, nonce12, ad);
    }

    static Cipher createForPigeon(Key privKey, Key pubKey, byte[] nonce12) {
        return Api.getProvider().cipherProvider().createCipherForPigeon(privKey, pubKey, nonce12);
    }
